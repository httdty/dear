...
def clean_data(data_1, data_1_fix, data_2, data_2_fix):
    all_data = []  # TODO: 改为extend，这个效率太低了
    for data in data_1:
        lines = data.split(" ")
    ...
    for data in data_2_fix:
        ...
        # for line in lines:
            # all_data.append(line)
    model = word2vec.Word2Vec([all_data], size=128, min_count=1)  # TODO: 这里应该要很久
    data_1_output = []
    data_1_fix_output = []
    data_2_output = []
    data_2_fix_output = []
...
def data_prepare(input_data):
    for idx in range(len(input_data)):
        ...
        for i in tqdm(range(len(input))):
            ...
            addon = np.zeros((inner_max_len - len(input[i]),128))
            input[i] = np.concatenate((input[i], addon), axis=0)
        input_data[idx] = np.array(input_data[idx])
